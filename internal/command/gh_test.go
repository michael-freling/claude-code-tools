...
func TestGhRunner_ListPRs(t *testing.T) {
	tests := []struct {
		...
		{
			name:   "successfully lists PRs with valid response",
			dir:    "/test/repo",
		...
		{
			name:   "returns empty list when no PRs found",
			dir:    "/test/repo",
			branch: "nonexistent-branch",
			setupMock: func(m *MockRunner) {
				m.EXPECT().
					RunInDir(gomock.Any(), "/test/repo", "gh", "pr", "list", "--head", "nonexistent-branch", "--json", "number,url,title,headRefName", "--limit", "1").
					Return(`[]`, "", nil)
			},
			want:    []PRInfo{},
			wantErr: false,
		...
		{
			name:   "returns error when gh command fails",
			dir:    "/test/repo",
			branch: "feature-branch",
			setupMock: func(m *MockRunner) {
				m.EXPECT().
					RunInDir(gomock.Any(), "/test/repo", "gh", "pr", "list", "--head", "feature-branch", "--json", "number,url,title,headRefName", "--limit", "1").
					Return("", "error: repository not found", fmt.Errorf("exit status 1"))
			},
			wantErr:     true,
