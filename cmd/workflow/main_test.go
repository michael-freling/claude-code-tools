...
func TestValidateStartFlags(t *testing.T) {
	tests := []struct {
		...
		{
			name:          "update-pr and split-pr together fails",
			skipTo:        "",
			withPlan:      "",
			updatePR:      123,
			forceBackward: false,
			setupFile:     false,
			wantErr:       true,
			wantErrMsg:    "invalid flag combination: cannot use --update-pr and --split-pr together",
		},
		{
			name:          "with-plan without skip-to fails",
			skipTo:        "",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := validateStartFlags(tt.skipTo, planPath, tt.updatePR, tt.forceBackward)
